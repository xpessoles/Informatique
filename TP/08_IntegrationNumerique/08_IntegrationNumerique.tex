...
\textit{En posant $k=\sin^2(\theta_0/2)$ et en effectuant le changement de variable $\sin(\theta/2)=k\sin\varphi$ montrer que }
$$
...
\textit{Écrire une fonction Python, appelée \textsf{Periode}, prenant en paramètres la période propre $T_0$ du
...
\subparagraph{}
\textit{Écrire une fonction, appelée \textsf{EcartRelatif}, prenant en paramètres la période propre $T_0$ et la période
...
\subparagraph{}
\textit{Écrire un programme Python permettant de créer trois listes nommées \textsf{ListeAngle}, \textsf{ListePeriode} et \textsf{ListeEcartRelatif} contenant respectivement :
\begin{itemize}
