...
\question{\'Ecrire une fonction \texttt{booleen(e:int,n:int)->str} qui convertit un entier en base 2 et renvoie le n\up{ième} bit sous la forme '0' ou '1' (n\up{ième} bit compté à partir de la droite).
...
\question{Vérifier que chacun des nombres de l'intervalle des entiers \verb![!0,32768\verb![! n'apparaissent qu'une seule fois sur une période. Pour cela, réaliser un programme qui affiche le nombre d'apparitions s'il est différent de 1, et qui affiche \textbf{Tous les nombres apparaissent une seule fois} si c'est le cas.\\ Conclure quant à l'équiprobabilité.}
