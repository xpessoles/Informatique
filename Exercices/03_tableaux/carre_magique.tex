...
\question{Proposer une fonction \texttt{def Carre\_vide(n:int) -> list :} qui crée et renvoie un carré magique vide (n listes remplies de n $0$), et qui renvoie une liste vide si $n$ est pair.}


\question{Proposer une fonction \texttt{def Remplir\_carre(CarreVide : list) -> None :} qui complète et renvoie un carré magique à partir d'un carré vide.}


\question{Proposer une fonction \texttt{def Verif\_carre(Carre : list) -> bool : } qui vérifie les trois propriétés d'un carré magique et renvoie un booléen (\texttt{True} ou \texttt{False})}
%}