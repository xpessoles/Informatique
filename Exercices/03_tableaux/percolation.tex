...
Nous aurons aussi besoin de la fonction \textbf{\pyv{rand}} du module \pyv{numpy.random}
...
fonction \pyv{np.zeros((n, p))} renvoie un tableau de $n$ lignes et
$p$ colonnes contenant dans chacune de ses cases le nombre flottant
0.0. Une fois un tableau \pyv{tab} créé, la case d'indice $(i, j)$ est
...
\pyv{plt.matshow} : appliquée à un tableau, celle-ci présente ce dernier sous
...
L'argument par défaut \pyv{cmap} de la fonction \pyv{plt.matshow} permet de modifier
l'échelle chromatique utilisée. La fonction \pyv{ListedColormap} va nous
