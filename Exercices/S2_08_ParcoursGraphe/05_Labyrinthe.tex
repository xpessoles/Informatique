...
\question{Écrire la fonction \cde{get\_aretes(G:dict) -> []} renvoyant la liste des arêtes du graphe sous la forme d'une liste de listes de tuples. Une arête est donc une liste de sommets où les sommets sont des tuples. Les arêtes ne devront être présentes qu'une fois. }
...
Dans notre cas, résoudre le labyrinthe consiste en partir du sommet \cde{(0,0)} (sommet en bas à gauche) et à se déplacer sur les arêtes dans le but d'atteindre le sommet en haut à droite. 
Pour générer un labyrinthe, nous allons réalisé un parcours de la grille \cde{G} (en largeur ou en profondeur). Le labyrinthe sera lui-même un graphe noté \cde{L}. À chaque fois qu'on \textbf{empilera ou enfilera} un \textbf{sommet non visité}, on ajoutera une arête entre entre ce sommet et le sommet père à \cde{L}. 




\question{Écrire la fonction \cde{ajouter\_arete(G:dict, s1:tuple, s2: tuple) -> None} qui permet d'ajouter l'arête \cde{([s1,s2])} au graphe \cde{G}. \textbf{On ajoutera cette arête uniquement si le sommet d'arrivée n'existe pas dans le graphe}. }
\begin{exemple}
...
\question{Écrire la fonction \cde{parcours\_largeur(G:dict, depart:tuple) -> dict} qui permet de créer un labyrinthe en largeur à partir d'un graphe \cde{G}. Tracer le labyrinthe obtenu. \cde{s} sera ici le point de départ du labyrinthe,\cde{(0,0)} par exemple.}
