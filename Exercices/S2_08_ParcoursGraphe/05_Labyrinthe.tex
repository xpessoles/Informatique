...
\question{Écrire la fonction \cde{creer\_graphe(n:int, p:int) -> \{\}} permettant de créer une grille de \cde{n} colonnes et \cde{p} lignes. }

On souhaite afficher ce graphe en utilisant \cde{matplotlib}. Pour cela, on va commencer par tracer chacune des arrêtes puis chacun des sommets. 
...
\question{Écrire la fonction \cde{get\_aretes(G:dict) -> list} renvoyant la liste des arêtes du graphe sous la forme d'une liste de listes de tuples. Une arête et donc une liste de sommets ou les sommets sont des tuples. Les arêtes ne devront être présentes qu'une fois. }
...
Pour générer un labyrinthe, nous allons réalisé un parcours de la grille (en largeur ou en profondeur). Le labyrinthe sera lui-même un graphe. Lors de ce parcours, à chaque fois qu'un sommet devra être visité. Ainsi à chaque fois qu'on découvrira un sommet à visiter, on ajoutera une arête entre le sommet père et ses voisins (s'ils ne sont pas dans le labyrinthe). 
...
Il est possible de résoudre le labyrinthe en utilisant un parcours en longueur ou un parcours en profondeur. 

\question{Écrire les fonctions \cde{resolution\_largeur(G:dict, s:tuple) -> list} qui permet de résoudre le labyrinthe en utilisant un parcours en largeur. Cette fonction renvoie la liste des sommets permettant d'atteindre le sommet en haut à droite depuis le sommet en bas à gauche.}
