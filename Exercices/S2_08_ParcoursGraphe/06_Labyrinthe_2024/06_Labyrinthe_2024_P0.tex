...
\begin{question}Écrire la fonction \lstinline|get_sommets(G:{}) -> ([],[])| renvoyant deux listes \lstinline{les_x} et \lstinline{les_y} contenant respectivement les abscisses des sommets et les ordonnées des sommets.
\end{question}
...
\begin{question}
Écrire la fonction \lstinline|trace_sommets(G:{}, couleur : str) -> None| qui trace sur la figure courante les sommets en utilisant un point coloré.
...
\begin{question}
Écrire la fonction \lstinline|get_aretes(G:{}) -> []| renvoyant la liste des arêtes du graphe sous la forme d'une liste de listes de tuples. Une arête est donc une liste de sommets où les sommets sont des tuples. Les arêtes ne devront être présentes qu'une fois.
\end{question}
...
\begin{question}
Écrire la fonction \lstinline{trace_arete(s1:(), s2:(), couleur : str, epaisseur : int ) -> None} qui trace une arête reliant les sommets \lstinline{s1} et \lstinline{s2} sur la figure courante. 
...
\begin{question}
Écrire la fonction \lstinline|trace_graphe(G:{},couleur : str,epaisseur : int) -> None| qui permet de tracer les sommets et les arêtes du graphe \lstinline{G} sur la figure courante. Tracer le graphe en rouge avec une épaisseur de 1 pour obtenir la figure ci-dessous.
\end{question}


On définit un chemin comme une liste de sommets. 

\begin{question}
Ecrire la fonction \lstinline|verif_chemin(G:{}, chemin : [()]) -> bool| qui renvoie \lstinline{True} si le chemin appartient au graphe, \lstinline{False} sinon.
\end{question}

\begin{question}
Écrire la fonction \lstinline|trace_chemin(G:{}, chemin:{}) -> None| qui permet de superposer le graphe et le chemin.
\end{question}
