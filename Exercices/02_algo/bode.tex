...
\begin{enumerate}
\item combien de données contiennent les listes \pyv{w}, \pyv{gain} et \pyv{phase} ?
...
\begin{tBox}
\question{} Pour la fonction proposée (\pyv{picResonance(w,gain,phase)}, proposer un invariant de boucle. Dans le cas où ce pic n'existerait pas, que renvoie la fonction ? Proposer un variant de boucle et démontrer que l'algorithme renvoie bien un résultat.
\end{tBox}
...
\begin{tBox}
\question{} Dans le cas où le diagramme en gain serait multi-pics, écrire la fonction \\ \pyv{picsResonance(w,gain,phase)} retournant une liste \pyv{L} de triplets de valeurs respectives une pulsation de résonance \pyv{wr}, le gain \pyv{gr} et la phase correspondante \pyv{pr} : \pyv{(wr,gr,pr)}. La liste \pyv{L} peut présenter l'allure suivante : \pyv{[(wr1,gr1,pr1),(wr2,gr2,pr2),...,(wrk,grk,prk)]}. Dans le cas où il n'y aurait aucun pics, la fonction retourne une liste vide.
\end{tBox}
