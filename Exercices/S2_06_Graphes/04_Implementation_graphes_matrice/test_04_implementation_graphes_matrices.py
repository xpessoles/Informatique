...
def test_Q09_01():
    A = [[1,2,3,4],[0,2],[0,1,3,4],[0,2,4],[0,2,3]]
    B = [[1,2,3,4],[0,2],[0,1,3,4],[0,2,4],[0,2,3]]
    A = ref.from_list_to_matrix(A)
    B = ref.from_list_to_matrix(B)
    assert A == B
    
def test_Q10_01():
    A = deepcopy(GG)
    B = deepcopy(GG)
    A = ref.from_matrix_to_list(A)
    B = eleve.from_matrix_to_list(B)
    assert A == B