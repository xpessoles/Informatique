...
\question{Implémenter la fonction \lstinline{creation_grille_vierge(n:int) -> [[]]} permettant de créer un tableau de \lstinline{n} lignes,  \lstinline{n} colonnes de 0. On utilisera pour cela deux boucles \lstinline{for} imbriquées. }
...
\question{Écrire une fonction \lstinline{a_un_voisin_rempli(grille:[[float]],i:int,j:int) -> bool} retournant \lstinline{True} si un des voisins est rempli, \lstinline{False} si aucun des voisins n'est rempli \marginnote{Un peu d'aide \url{https://ptsilamartin.github.io/info/TP/Q08.py}.}
