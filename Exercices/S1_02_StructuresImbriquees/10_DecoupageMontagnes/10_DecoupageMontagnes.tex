...
\question{Écrire la fonction \texttt{maximum(L:list) -> int} permettant de déterminer le maximum d'une liste (la fonction \texttt{max} sera ici interdite).}
\ifprof
\begin{corrige}~\\
\vspace{-.5cm}
\begin{lstlisting}
...
\question{Vérifier que \texttt{maximum(les\_y[0:300])} renvoie \texttt{1855.99}.}

\question{Écrire la fonction \texttt{plus\_haut\_indice(L:list) -> float} permettant de déterminer l'indice de l'altitude la plus haute atteinte lors de la randonnée.}
\ifprof
\begin{corrige}~\\
\vspace{-.5cm}
\begin{lstlisting}
...
\question{Vérifier que \texttt{plus\_haut\_indice(les\_y[300:500])} renvoie \texttt{199}.}


\question{Écrire la fonction \texttt{deniveles(alt:list) -> list} qui calcule les dénivelés cumulés positif et négatif (en mètres) de la randonnée, sous forme d’une liste de deux flottants. Le dénivelé positif est la somme des
...
\question{Vérifier que \texttt{deniveles(les\_y)} renvoie \texttt{[3438.100, -2746.747]}.}
...
\question{Écrire la fonction \texttt{moyenne(alt:list) -> float} permettant de calculer la moyenne des altitudes mesurées par le GPS (l'utilisation de \texttt{sum} est interdite).}
\ifprof
\begin{corrige}~\\
\vspace{-.5cm}
\begin{lstlisting}
...
\question{Vérifier que \texttt{moyenne(les\_y)} renvoie \texttt{1376.35}.}


\question{On note \texttt{n} la taille du tableau \texttt{les\_y}. Créer une liste \texttt{les\_moy} contenant \texttt{n} fois la valeur \texttt{moyenne(les\_y)}. Tracer sur la même courbe le profil de la randonnée et la valeur moyenne sur tout le profil. }
...
\question{Écrire la fonction \texttt{indice\_premier\_PNM(alt:list) -> int} renvoyant, s’il existe, l’indice \texttt{i}
...
\question{Vérifier que \texttt{indice\_premier\_PNM(les\_y)} renvoie \texttt{53} et  
\texttt{indice\_premier\_PNM(les\_y[200:250])} renvoie \texttt{-1}.}




\question{Écrire la fonction \texttt{indices\_PNM(alt:list) -> list} retournant la liste des indices de tous les PNM.}
...
\question{Vérifier que \texttt{indices\_PNM(les\_y)} renvoie \texttt{[53, 449, 889]}.}


\question{Dans le but de séparer les différents profils, nous allons chercher les indices des altitudes minimales entre deux PNM successifs. Écrire la fonction \texttt{liste\_alt\_mini(alt:list) -> list} qui répond à ce besoin. En utilisant le profil donné, cette fonction renverrait la liste  \texttt{[300,826]}.}
\ifprof
\begin{corrige}~\\
\vspace{-.5cm}
\begin{lstlisting}
...
\question{Écrire la fonction \texttt{creer\_montagnes(alt) -> list} renvoyant une liste constituée de la liste des montagnes élémentaires.}
\ifprof
\begin{corrige}~\\
\vspace{-.5cm}
\begin{lstlisting}
...
\question{Tracer chacun des montagnes afin d'obtenir le graphique suivant.}
%\ifprof
%\else
\begin{figure}[H]
\centering
...
%
%
%\question{Écrire la fonction \texttt{voisins(p:list) -> list} qui construit la liste des positions
%voisines de~\texttt{p}.}
%\ifprof
%\begin{corrige}~\\
%\vspace{-.5cm}
...
%\fi
%
%\question{Écrire la fonction \texttt{positions\_possibles(p:list, atteints:list)-> list} qui construit la liste des positions
...
%\fi
%
%\question{Écrire la fonction \texttt{genere\_chemin\_naif(n:int) -> list } qui construit la liste des points représentant le 
...
%
%
%\question{Écrire la fonction \texttt{est\_CAE(chemin)} qui vérifie si un chemin est auto-évitant en se basant sur sorted et renvoie un résultat booléen. Elle devra ne pas être de complexité temporelle asymptotique
...
%
%
%\question{Écrire la fonction \texttt{rot(p, q, a)} qui renvoie le point image du point \texttt{q} par la rotation de centre \texttt{p} et d’angle défini par la valeur de \texttt{a} : 0 pour $\pi$, 1 pour $\dfrac{\pi}{2}$, 2  pour
%$-\dfrac{\pi}{2}$.}
%\ifprof
%\begin{corrige}~\\
%\vspace{-.5cm}
...
%\fi
%
%\question{Écrire la fonction \texttt{rotation(chemin, i\_piv, a)} qui renvoie un nouveau chemin identique à chemin jusqu’au pivot d’indice \texttt{i\_piv}, et ayant subi une rotation de a autour du pivot (même codage que
...
%\fi
%
%\question{Écrire la fonction \texttt{genere\_chemin\_pivot(n, n\_rot)} permettant de générer un chemin autoévitant de longueur \texttt{n} en appliquant \texttt{n\_rot} rotations. L’application d’une rotation peut nécessiter plusieurs tentatives.}
%\ifprof
%\begin{corrige}~\\
%\vspace{-.5cm}
%\begin{lstlisting}
