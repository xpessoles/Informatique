%% SUR LA VERSION 2025, on ajoute la copie d'une imagz

Avant toute chose, créer un répertoire \lstinline{TP09} dans votre répertoire \lstinline{Informatique}, \lstinline{TPsup} dans votre espace personnel.

Sur le site \url{https://ptsilamartin.github.io/}, télécharger les 2  images \href{https://ptsilamartin.github.io/TP/allium.png}{allium.png}
et \href{https://ptsilamartin.github.io/TP/ciel.png}{ciel.png} ainsi que le script \href{https://ptsilamartin.github.io/TP/TP_09.py}{TP\_09.py}  que vous copiez dans votre répertoire TP09. Ouvrir le script \lstinline{TP_09.py} dans lequel vous travaillerez tout au long de ce TP.
...
\begin{question}
Compléter les commentaires de la fonction \lstinline{image_noire} fournie dans le script \lstinline{TP09.py}. Appeler ensuite cette fonction pour créer une image noire de largeur 6 pixels et de hauteur 3 pixels.
\end{question}
...
\section*{Copier une image}
Dans la suite du TP, nous allons modifier des images. Pour ne pas altérer l'image d'origine, on devra la copier puis modifier la copie. 

\begin{question}
\'Ecrire la fonction \lstinline{copie} qui prend pour argument la matrice-image \lstinline{im}  et qui crée une copie indépendante. 
L'utilisation de \lstinline{copy} ou \lstinline{deepcopy} est strictement interdite.
\end{question}
...
Attention, l'image iniale ne devra pas être modifiée.
\end{question}
...
\begin{question}
\'Ecrire une fonction \lstinline{contour}, qui prend comme argument une image en niveaux de gris \lstinline{im} ainsi qu'un flottant \lstinline{seuil}, et qui renvoie une image noire sauf les pixels situés sur le contour qui seront en blanc.\sidenote{Les pixels appartenant aux bords de l'image ne seront pas traités.}
\end{question}
