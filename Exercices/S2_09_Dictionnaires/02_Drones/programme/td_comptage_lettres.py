...
def compterLettre(texte:str):  # definition de la fonction
    dictionnaire={}             # creation d'un dictionnaire vide
    for lettre in texte:        # parcours des caractères du texte
        if lettre in 'azertyuiopqsdfghjklmwxcvbn': # si le caractère est une lettre minuscule non accentuée de l'alphabet
            if lettre in dictionnaire.keys(): # si cette lettre est une clé du dictionnaire
                dictionnaire[lettre]+=1 # on incrémente sa valeur de 1
            else:
                dictionnaire[lettre]=1 # sinon, on crée la clé et on lui associe la valeur 1
    return dictionnaire             # le dictionnaire est renvoyé



# >>> compterLettre(texte)
# {'i': 105, 'l': 84, 'e': 175, 's': 102, 't': 93, 'p': 39, 'a': 102, 'r': 75, 'v': 24, 'n': 139, 'u': 99, 'm': 30, 'd': 40, 'f': 15, 'c': 40, 'o': 87, 'h': 10, 'x': 7, 'b': 8, 'q': 23, 'g': 10, 'j': 1, 'y': 4}
