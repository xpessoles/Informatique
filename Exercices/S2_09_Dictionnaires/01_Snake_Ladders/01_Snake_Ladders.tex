...
%	\hline
%	
%	Longueur (\texttt{len(L)}) &  $O(1)$ \\
%	\hline
%	Accès en lecture d'un élément &  $O(1)$ \\
%	\hline
%	Accès en écriture d'un élément &  $O(1)$ \\
%	\hline
%	Copie (\texttt{L.copy()} ou \texttt{L[:]}) & $O(n)$ \\
%	\hline
%	Ajout (\texttt{L.append(elt)} ou \texttt{L+=[elt]}) & $O(1)$ \\
%	\hline
%	Extension (\texttt{L1.extend(L2)} ou \texttt{L1+=L2}) & $O(n_2)$ \\
%	\hline
%	Concaténation (\texttt{L1 + L2}) &  $O(n_1 + n_2)$ \\
%	\hline
%	Test de présence (\texttt{elt in L}) & $O(n)$ \\
%	\hline
%	Désempiler dernier (\texttt{L.pop()}) &  $O(1)$ \\
%	\hline
%	Désempiler autre (\texttt{L.pop(-k)}) &  $O(k)$ \\
%	\hline
%	Maximum ou minimum (\texttt{max(L)} et \texttt{min(L)}) &  $O(n)$ \\
%	\hline
%	Tri (\texttt{L.sort()} ou \texttt{sorted(L)}) &  $O(n \log(n))$ \\
%	\hline
%\end{tabular}
%
...
%	 Opération & Moyen \\
%	\hline
%	Longueur (\texttt{len(d)}) &  $O(1)$ \\
%	\hline
%	Accès en lecture d'un élément (\texttt{x = d[k]})  &  $O(1)$ \\
%	\hline
%	Accès en écriture d'un élément (\texttt{d[k] = x}) &  $O(1)$ \\
%	\hline
%	Copie (\texttt{d.copy()}) & $O(n)$ \\
%	\hline
%	Ajout (\texttt{d[k] = x} la première fois) & $O(1)$ \\
%	\hline
%	Test de présence (\texttt{k in d}) & $O(1)$ \\
%	\hline
%	Retrait d'un élément (\texttt{del d[k]} ou \texttt{d.pop(k)}) & $O(1)$ \\
%	\hline
%\end{tabular}
