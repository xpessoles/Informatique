...
d,vd=trapeze(V2,tp)
plt.figure(figsize=(20,10))
plt.title('Affichage de la distance parcouru')
plt.subplot(3,1,1)
#plt.plot(tp,Pa,label="Inertie")
plt.plot(tp,vd,label="Pesanteur")
plt.xlabel('temps (s)')
...
M=80
g=9.81
Pa=M*(ar2*vr2+atheta2*vtheta2+aphi2*vphi2)#Puissance inertielle
Pa=M*V2*dif(V2,tp)#Puissance inertielle
Pg=M*g*V2*np.sin(np.arctan(dif(rf,vd)))#Puissance de pesanteur

#Puissance du vent
rho0=1.293
T=293
alt=0.3
PB=760*np.exp(-0.124*alt)
rho=rho0*273*PB/(760*T)#Masse volumique de l'air
thetav=2*np.pi
uv_theta=np.cos(thetav)
uv_phi=np.sin(thetav)
vf=4.8#Vitesse du vent
vvf=vf*np.array([np.cos(thetav)*np.ones(len(V2)),np.sin(thetav)*np.ones(len(V2))])
Vc=np.array([vphi2,vtheta2])
Vapp=vvf-Vc
Vapp_carr=np.zeros([len(Vapp[0])])
for k in range(len(Vapp[0])):
    Vapp_carr[k]=Vapp[0,k]**2+Vapp[1,k]**2
ApCd=0.221
Pvent=0.5*rho*ApCd*Vapp_carr*(vphi2*uv_phi+vtheta2*uv_theta)

Pt=Pa+Pg#Puissance totale
for i in range(len(Pt)):
    if Pt[i]<0:
        Pt[i]=0

plt.figure(figsize=(20,10))
plt.title('Affichage de la puissance')
plt.subplot(3,1,1)
plt.plot(tp,Pa,label="Inertie")
plt.plot(tp,Pg,label="Pesanteur")
plt.plot(tp,Pvent,label="Vent")
plt.grid()
plt.xlabel('temps (s)')
plt.ylabel('$P$ (en W)')
plt.legend(loc=0)
plt.subplot(3,1,2)
plt.plot(tp,Pt,label="Puissance totale")
plt.grid()
plt.xlabel('temps (s)')
plt.ylabel('$P$ (en W)')
plt.legend(loc=0)
...
#Estimation de lenergie
Et=trapeze(Pt,tp)[-1][-1]*0.000239006
print('Energie dépensé en kcal : '+str(Et))
