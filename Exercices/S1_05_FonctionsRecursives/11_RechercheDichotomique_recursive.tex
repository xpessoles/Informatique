...
\question{\'Ecrire une fonction \texttt{dichotomie$\_$rec(x0,L)} qui renvoie \texttt{True} ou \texttt{False} selon que \texttt{x0} figure ou non dans \texttt{L} par cette méthode. On utilisera une boucle \texttt{while} que l'on interrompra soit lorsque l'on a trouvé \texttt{x0}, soit lorsque l'on a fini de parcourir la liste.}
