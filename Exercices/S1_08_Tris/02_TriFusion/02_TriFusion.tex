...
\question{Implémenter la fonction \lstinline{separe(L : []) -> ([],[])} renvoyant deux listes de longueurs quasi-égales.}
\ifprof
\begin{corrige}
\begin{lstlisting}
def separe(L: []) ->([],[]):
...
\question{Implémenter la fonction \lstinline{fusion(L1:[], L2:[]) -> []} réalisant la fusion de deux listes triées. Pour cela on procèdera ainsi :
\begin{enumerate}
...
\question{Implémenter la fonction \lstinline{tri_fusion(L : []) -> []} réalisant par récursivité le tri de la liste \lstinline{L}.}
