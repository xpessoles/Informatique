...
\question{\'Ecrire une fonction \texttt{dichotomie(x0,L)} qui renvoie \texttt{True} ou \texttt{False} selon que \texttt{x0} figure ou non dans \texttt{L} par cette méthode.}

\question{Dans le pire des cas où \texttt{x0} ne figure pas dans \texttt{L}, donner un équivalent du nombre de comparaisons qui sont effectuées dans cette fonction. Comparer avec le nombre de comparaisons à faire dans le cas d'un algorithme de recherche séquentielle.}
