...
\begin{figure}[h]
\centering
\includegraphics{SacEtFruits.png}
...
Un fruit est représenté par une liste \texttt{fruit=\verb![!prix au kilo,nom,quantité ramassée\verb!]!}
\begin{center}
cueillette = \verb![!\verb![!24,"framboises",1\verb!]!, \verb![!16,"myrtilles",3\verb!]!, \verb![!6,"fraises",5\verb!]!, \verb![!3,"mures",2\verb!]!\verb!]!
\end{center}

\question{Dans \textbf{Algorithmes\_gloutons.py}, compléter la fonction \texttt{sac\_a\_dos(L:list, capacite:int)} qui prend en argument une liste de listes \texttt{L} modélisant la cueillette et la capacité du sac à dos, et appliquant la méthode décrite précédemment. Cette fonction renvoie :}
\textit{
\begin{itemize}
...
\question{A la suite du script \textbf{Algorithmes\_gloutons.py}, taper les lignes de code permettant d’utiliser la fonction \texttt{sac\_a\_dos( )} pour afficher les  fruits et les quantités à choisir pour remplir le sac à dos, à partir de la liste \texttt{cueillette}. Exécuter votre programme et vérifier le résultat.}
...
\question{Dans \textbf{Algorithmes\_gloutons.py}, copier-coller \texttt{sac\_a\_dos(L , capacite)} que vous renommerez \texttt{sac\_a\_dos\_V2(L , capacite)} et adapter la fonction de sorte qu’elle applique la méthode gloutonne à la version non fractionnaire du problème. Pour cela, il faut :
%\begin{itemize}
...
\question{Le résultat obtenu est-il optimal ? Comparer avec la solution S = \verb![!\verb![!2.5,"melon jaune",2,1\verb!]!, \verb![!2,"pastèque",3,1\verb!]!\verb!]!. Quelle est la solution dont la valeur est maximale ?}
