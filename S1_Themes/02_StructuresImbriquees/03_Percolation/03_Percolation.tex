...
Afin de remplir la grille, on dispose de la fonction \texttt{rd.random()} permettant de renvoyer un nombre aléatoire compris dans l'intervalle $[0,1[$.
...
\question{Définir une fonction \texttt{Python}, de signature \texttt{def creation\_grille(p: float, n: int) -> list :
} à deux paramètres : un nombre réel $p$ (qu'on supposera dans l'intervalle $[0,1[$ et un entier naturel $n$, qui renvoie un tableau $(n,n)$ dans lequel chaque case sera ouverte avec la probabilité $p$ et fermée sinon. }
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{proba}
