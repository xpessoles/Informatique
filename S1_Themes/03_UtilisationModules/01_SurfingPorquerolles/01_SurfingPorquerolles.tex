...
\question{Écrire une fonction  \\
\texttt{proprietes(liste\_niveaux: list) -> list}
réalisant cet objectif. On pourra
utiliser les fonctions de Python \texttt{max(L)} et \texttt{min(L)} qui retournent le maximum et le minimum d’une
...
\question{Écrire une fonction  \\
****
prenant en argument la liste liste\_niveaux de la question 12*** et
renvoyant $H_{max}$.}
...
%% CODER TOUTES CES FONCTIONS
\question{Un codage de la fonction \texttt{skewness} pour une liste ayant au moins 3 éléments est donné
en annexe (algorithme 4). Le temps d’exécution est anormalement long. Proposer une modification
simple de la fonction pour diminuer le temps d’exécution (sans remettre en cause le codage des
fonctions \texttt{ecartType} et \texttt{moyenne}).}