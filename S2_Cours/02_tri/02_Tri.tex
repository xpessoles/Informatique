...
\hline
Tableau $T$ initial: 5 est la clef. & \cellcolor{bleu}5 & 14 & 11 & 8 \\ 
\hline 
\hline \rowcolor{white}
La clef est comparée avec la liste des valeurs déjà triées (vide ici). & \cellcolor{rouge}5 & 14 & 11 & 8   \\ 
\hline 
\hline \rowcolor{white}
 La clef est 14 & \cellcolor{rouge}5 & \cellcolor{bleu}14 & 11 & 8  \\ 
\hline 
\hline \rowcolor{white}
On compare la clef aux valeurs déjà triées (ici 5). Elle est plus grande et reste donc à sa position. & \cellcolor{rouge}5 & \cellcolor{rouge}14 & 11 & 8  \\ 
\hline 
\hline \rowcolor{white}
 La clef est 11. & \cellcolor{rouge}5 & \cellcolor{rouge}14 & \cellcolor{bleu}11 & 8  \\ 
\hline 
\hline \rowcolor{white}
On compare la clef aux valeurs déjà triées : la première valeur comparée est 14. 14 est plus grand donc on décale 14 vers la droite pour laisser une place à la clef. & \cellcolor{rouge}5 & \cellcolor{vert}14 & \cellcolor{vert}14 & 8 \\ 
\hline 
\hline \rowcolor{white}
On compare la clef aux valeurs déjà triées : la valeur suivante est 5. 5 est plus petit donc on a trouvé la place de la clef et on l'écrit. & \cellcolor{rouge}5 & \cellcolor{rouge}11 & \cellcolor{rouge}14 & 8  \\ 
\hline 
\hline \rowcolor{white}
La clef est 8. & \cellcolor{rouge}5 & \cellcolor{rouge}11 & \cellcolor{rouge}14 & \cellcolor{bleu}8  \\ 
\hline 
\hline \rowcolor{white}
On compare la clef 8 à 14. 14 est plus grand donc on le décale en le copiant à la place de la clef. & \cellcolor{rouge}5 & \cellcolor{rouge}11 & \cellcolor{vert}14 & \cellcolor{vert}14  \\ 
\hline 
\hline \rowcolor{white}
On compare la clef 8 à 11. 11 est plus grand donc on le décale en le copiant à la place de la valeur supérieure (on écrase donc une des occurrences de 14). & \cellcolor{rouge}5 & \cellcolor{vert}11 & \cellcolor{vert}11 & \cellcolor{rouge}14  \\ 
\hline 
\hline \rowcolor{white}
On compare la clef  8 à 5. 5 est plus petit donc on est à la bonne place pour copier la clef (on écrase donc l'occurrence de 11 en doublon). & \cellcolor{rouge}5 & \cellcolor{rouge}8 & \cellcolor{rouge}11 & \cellcolor{rouge}14  \\ 
\hline 
\end{tabular} 
