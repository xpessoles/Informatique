...
for i in range (len(temps)-1):
    vitesse_rec=vitesse_rec+[vitesse_rec[i]+acceleration[i]*pas_temps*3.6] # 3.6 : conversion en km/h

# méthode des trapezes
pas_temps=temps[1]-temps[0]   # calcul du pas temporel
vitesse_trap=[0] # vitesse initiale
for i in range (len(temps)-1):
    vitesse_trap=vitesse_trap+[vitesse_trap[i]+0.5*(acceleration[i+1]+acceleration[i])*pas_temps*3.6] # 3.6 : conversion en km/h
