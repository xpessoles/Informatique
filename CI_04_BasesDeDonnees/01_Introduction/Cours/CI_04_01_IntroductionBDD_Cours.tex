...
Il existe deux possibilités pour interroger une base de donnée au format SQL. Il est d'abord possible d'utiliser des interfaces graphiques qui vont permettre d'interroger une base de donnée avec le langage naturel. 
\begin{exemple}

\end{exemple}

Il est aussi possible d'utiliser le langage SQL directement pour faire des opérations sur une base. 
...
\texttt{CREATE DATABASE Aeroports;}
...
\texttt{CREATE TABLE nom\_table (nom\_attribut\_1 type\_attribut1, nom\_attribut\_2  type\_attribut\_2, ...);}

\begin{exemple}
\texttt{CREATE TABLE Pays (Identifiant INT, Code CHAR, Nom CHAR);}
\end{exemple}
...
\begin{exemple}
\texttt{CREATE TABLE Pays (Identifiant INT UNIQUE PRIMARY KEY , Code CHAR NOT NULL , Nom CHAR NOT NULL);}
\end{exemple}
...
\begin{sql}
SELECT code,name FROM Countries UNION code,name FROM Regions
\end{sql}

La relation résultant contient donc l'ensemble des couples (code,name) des régions et des  
\end{envsql}
