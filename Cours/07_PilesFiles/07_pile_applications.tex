...
\subparagraph*{La parenthèse inattendue}
...
\subparagraph*{Réaliser un programme permettant de savoir si une chaîne de caractères est bien parenthésée. La structure de pile est-elle nécessaire ?}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\subparagraph*{Dépile le n\ieme}
%\setcounter{exo}{0}

\question{Écrire une fonction qui dépile et renvoie le n\ieme élément d’une pile de taille au moins
égale à $n$. Le premier élément correspond au sommet de la pile. Les $n-1$ premiers éléments devront conserver leur place.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\subparagraph*{Inversion des extrêmes}
...
\subparagraph*{Inversion de la pile}
%\setcounter{exo}{0}
...
\question{Si on s’autorise à détruire la pile initiale, quelle est la complexité en temps et en espace de cette fonction ? Et si on ne s’y autorise pas ?}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\begin{exemple}
Si la pile initiale est \texttt{[1, 2, 3, 4, 5]}, et que le nombre d’éléments retirés vaut 2, alors la pile ne contient plus que \texttt{[1, 2, 3]} et la pile renvoyée contient \texttt{[5,4]}.
\end{exemple}
\ifprof
\begin{corrige}
\end{corrige}
