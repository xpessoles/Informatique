...
\caption{Évolution du nombre d'opérations pour rechercher un nombre dans une liste de 1 à 1000 nombres}
\end{figure}
...
\begin{defi}[Complexité algorithmique -- Notation $\mathcal{O}$ -- \cite{Bournez}]
...
Chacune de ces 3 instructions ont le même coût temporel $C_e$.
...
\begin{resultat}
Le coût d'un test \lstinline{if test : inst_1 else : inst_2 } est inférieur ou égal au maximum du coût de l'instruction 1 et du coût de l'instruction 2 additionné au coût du test (coût élémentaire). 
\end{resultat}
...
\lstinline{foo} prendra (a priori) toujours le même temps d'exécution. On a donc $\indice{C}{foo}(n) = \mathcal{O}(1)$. On parle de complexité constante. 
\end{exemple}


\begin{resultat}
Le coût d'une boucle \lstinline{for i in range(n) : inst}  est égal à : $n$ fois le coût de l'instruction \lstinline{inst} si elle est indépendante de la valeur de $i$.
\end{resultat}
...
On peut considérer que
 l'incrémentation de \lstinline{i} a un coût $C_e$. 
...
\begin{resultat}
Soit la boucle \lstinline{while cond : inst}, la condition \lstinline{cond} faisant intervenir un variant de boucle. Il est donc possible de connaître le nombre $n$ d'itérations de la boucle. 
...
\begin{exemple}[Recherche d'un maximum]
...
Ainsi, l'instruction \lstinline{e in L} est linéaire $\mathcal{O}(n)$ car Python dont inspecter chacun des éléments d'une liste de taille \lstinline{n} pour savoir si \lstinline{e} est dedans ou non.

Il en est de même pour l'instruction \lstinline{copy}. 

Concernant la méthode \lstinline{append} on pourra faire l'hypothèse que cela s'effectue en temps constant ($\mathcal{O}(1)$). Cependant, lorsqu'on ajoute une liste Python préréserve des espaces d'allocations juxtaposés. Tant que la liste ne dépasse pas la taille de l'espace alloué, l'ajout d'un élément se fait à temps constant. Quand cette taille est dépassée, la liste est déplacée à un autre endroit en mémoire où plus d'espace est nécessaire. Cette copie se fait donc en temps linéaire. Ainsi, pour append on parle de temps amorti constant.
...
\lstinline{cProfile} renvoie alors le message suivant :
...
On a alors le bilan du temps passé à effectuer chacune des opérations. Ainsi pour améliorer notablement l'algorithme, le plus intéressant serait d'optimiser la méthode \lstinline{remove}.
\end{exemple}


\section*{Bibliographie}
\begin{enumerate}[label={[\arabic*]}]
\item {François Denis \url{http://pageperso.lif.univ-mrs.fr/~francois.denis/algoL2/chap1.pdf}}
\item {Alain Soyeur \url{http://asoyeur.free.fr/}}
\item {François Morain, Cours de l'Ecole Polytechnique, \url{http://www.enseignement.polytechnique.fr/profs/informatique/Francois.Morain/TC/X2004/Poly/www-poly009.html}.}
\item {Renaud Kerivent et Pascal Monasse, La programmation pour ... , Cours de l’École des Ponts ParisTech - 2012/2013 \url{http://imagine.enpc.fr/~monasse/Info}.}
\item  {Olivier Bournez, Cours INFO 561 de l'Ecole Polytechnique, Algorithmes et programmation, \url{http://www.enseignement.polytechnique.fr/informatique/INF561/uploads/Main/poly-good.pdf}.}
\item {Wack et Al., \textit{L'informatique pour tous en classes préparatoires aux grandes écoles}, Editions Eyrolles.}
\end{enumerate}
