...
\exer{Somme récursive }
\setcounter{question}{0}
\question{Écrire une fonction récursive \lstinline{somme_n(n)} permettant de calculer la somme des éléments de 1 à $n$.}
\ifprof
\begin{lstlisting}
def somme(n):
    if n == 0 :
...
\exer{Somme récursive des éléments d'une liste}
\setcounter{question}{0}
\question{Écrire une fonction récursive \lstinline{somme(L)}, de paramètre une liste \lstinline{L} de nombres, qui calcule et renvoie
la somme des éléments de \lstinline{L}. On précise que l’exécution de \lstinline{somme([])} doit renvoyer la valeur \lstinline{0} et que
la liste \lstinline{L} n’est pas modifiée par la fonction (elle peut être modifiée momentanément mais, à la fin de
l’exécution de la fonction, on doit retrouver la liste initiale.}



\exer{Maximum récursif}
\setcounter{question}{0}
\question{Écrire une fonction récursive \lstinline{maximum(L)}, de paramètre une liste \lstinline{L} de nombres (supposée non vide),
qui détermine et renvoie le maximum de \lstinline{L}. On précise que la liste \lstinline{L} n’est pas modifiée par la fonction
et qu’il ne doit bien sûr pas être fait usage de la fonction native max pour les listes.}
...
\question{Ecrire une fonction itérative \lstinline{expo_rap_it(x:int, n:int)->int} permettant de calculer $x^n$ (sans utiliser \lstinline{**}).}


\question{Ecrire une fonction récursive \lstinline{expo_rap_rec(x:int, n:int)->int} permettant de calculer $x^n$ (sans utiliser \lstinline{**}).}


\exer{Dessins récursifs}
\setcounter{question}{0}
\marginnote{oooooo \\
ooooo\\
oooo\\
...
oo\\
o}
\question{Écrire une fonction récursive \lstinline{triangle_1(n)}, de paramètre un entier naturel \lstinline{n}, qui ne
renvoie rien mais qui produit à l’aide de la fonction \lstinline{print} la figure ci-contre (dans le cas bien sûr où
la fonction est évaluée en 6).}
...
ooooo \\
oooooo}
\question{Faire de même avec une fonction récursive \lstinline{triangle_2(n)} qui produit ce même «
triangle rectangle » mais pointe en haut (ci-contre la figure obtenue dans le cas où la fonction est
évaluée en 6).}
%
%
%\question{Énoncer un variant de boucle et montrer la terminaison de l'algorithme.}
...
%
%\exer{Triangles de Sierpinski}
%\setcounter{question}{0}
%
%%\subparagraph{}\textit{}
%On suppose disposer d’une fonction \lstinline{polygon([xa, ya], [xb, yb], [xc, yc])} qui trace le
