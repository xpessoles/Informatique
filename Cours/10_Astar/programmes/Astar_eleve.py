...
def distance_man(l,c,la,ca):
    '''Renvoie la distance de Manhattan entre deux points de coordonnees (l,c) et(la,ca)'''
    diag=min(abs(l-la),abs(c-ca))
    return 14*diag+((abs(c-ca)-diag)+(abs(l-la)-diag))*10



def f_heuristique(l,c,la,ca):
    Dfin=distance_man(l,c,la,ca)
    Ddebut=0
    return Dfin+Ddebut
