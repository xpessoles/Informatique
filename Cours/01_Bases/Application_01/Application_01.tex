...
\question{Implémenter la fonction \texttt{zeros\_03(n:int)->list} permettant de générer une liste de \texttt{n 0}. On utilisera une boucle \texttt{while}. On génera cette liste << en compréhension >> (sans boucle \texttt{for} ou \texttt{while} explicite).}
...
\question{Implémenter la fonction \texttt{entiers\_03(n:int)->list} permettant de générer la liste des entiers compris entre 0 inclus et \texttt{n} exclus. On génera cette liste << en compréhension >> (sans boucle \texttt{for} ou \texttt{while} explicite).}
