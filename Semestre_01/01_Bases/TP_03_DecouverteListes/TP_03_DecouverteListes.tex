...
\question{Calculer la moyenne et l'écart-type (racine carrée de la variance) de la liste \texttt{les_notes}.

<i> Remarque </i>: On trouvera une moyenne d'environ $9.08$ et un écart-type d'environ $2.93$.
    </span>
</div>
...
\question{Ecrire une fonction \texttt{moyenneGlissante(a:list, n:int)->list} d'argument une liste \texttt{a} d'entiers ou flottants et un entier \texttt{n} et renvoyant la liste des moyennes glissantes sur \texttt{n} éléments consécutifs.<br/>
        Vous pourrez avantageusement utiliser le slicing.<br/>
        Faire un test avec la listes \texttt{les_notes}.}
...
\question{Ecrire une fonction \texttt{majores_par(L:list,x:float)->int} réalisant cette opération.}
...
\question{Modifier la fonction précédente pour obtenir une fonction \texttt{ elements_majores_par(L:list,x:float)->list}
...
\question{Définir une fonction \texttt{sequentielle(a:list,x:float)->bool}, d'arguments
  ...
  Faire vos tests avec la liste \texttt{les_notes} en prenant pour \texttt{x} la première valeur de \texttt{les_notes}, puis la dernière et enfin une valeur au milieu. }
...
\question{Définir une fonction \texttt{occurrenceListe(a:list)->list}, d'argument
  une liste \texttt{a} d'entiers compris dans l'intervalle [0,k] et de longueur \texttt{n} tel que $k<n$ et qui renvoie la liste des nombres d'occurrence des entiers de la liste \texttt{a}. Cette valeur est 0 si le nombre n'est pas dans la liste.<br/>
<i>Remarque</i> : Une seule boucle for est acceptée.}
...
\question{Ecrire une fonction \texttt{hasard_liste(n:int,k:int)->list} d'arguments deux entiers \texttt{n} et \texttt{k} permettant de générer une liste de \texttt{n} entiers aléatoires appartenant à \texttt{range(k)}.}
...
\question{Créer une liste à partir de la fonction \texttt{hasard_liste(n:int,k:int)} et tester votre fonction \texttt{occurrenceListe(a:list)}.}
