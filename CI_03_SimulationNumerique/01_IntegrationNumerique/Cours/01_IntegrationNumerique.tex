...
\left(a_{i+1}-a_{i}\right) \cdot f\left(\dfrac{a_i+a_{i+1}}{2}\right) \\
 ...
 + \underbrace{\int\limits_{a_i}^{a_{i+1}} \left( x - \dfrac{a_i+a_{i+1}}{2}\right)\;\mathrm{d}x}_{0} \\%\quad \text{à comprendre} \\
\end{eqnarray*}
...
Soit $f$ fonction $\mathcal{C}^2$ sur $I=[a,b]$. %Soit $M$ un majorant de $f''$ sur $I$. 
...
\bibitem{1}{Adrien Pétri, \textit{Analyse numérique : Intégration numérique}, Notes de cours de TSI 1, Lycée Rouvière, Toulon.}
\end{thebibliography}
\end{document}
