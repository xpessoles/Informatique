...
\subsection{Calcul d'erreur -- Rectangles à gauche}
...
On note $\mathcal{A}_i = \int\limits_{a_i}^{a_{i+1}}f(x)\mathrm{d}x $. Par la méthode des rectangles à gauche, $\mathcal{A}_i$ est approximée par $\left(a_{k+1}-a_{k}\right) f(a_k) $. 

\begin{Objectif}
Déterminer sur chaque intervalle $[a_i,a_{i+1}]$ l'erreur commise par une intégration utilisant la méthode des rectangles à gauche. On va pour cela chercher à majorer l'erreur $\varepsilon_k$ :
$$
\varepsilon_k = \left| \int\limits_{a_i}^{a_{i+1}}f(x)\mathrm{d}x -\left(a_{i+1}-a_{i}\right) f(a_i)\right|
$$
\end{Objectif}
...
\begin{eqnarray*}
\varepsilon_k &\leq &  \left| \int\limits_{a_i}^{a_{i+1}}f(x) - \left(a_{i+1}-a_{i}\right) f(a_i) \; \mathrm{d}x\right|  \\
...
&\leq &   \int\limits_{a_i}^{a_{i+1}} \underset{[a,b]}{\sup} \left| f'(x)\right| \cdot \left| x-a_i\right|  \; \mathrm{d}x \quad \text{d'après l'inégalité des accroissements finis}\\
\end{eqnarray*}
...
\bibitem{1}{Adrien Pétri, \textit{Analyse numérique : Intégration numérique}, Notes de cours TSI 1, Lycée Rouvière, Toulon.}
\end{thebibliography}
\end{document}
