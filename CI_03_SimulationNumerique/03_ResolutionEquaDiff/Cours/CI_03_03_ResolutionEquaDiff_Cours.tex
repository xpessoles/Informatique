...
On modélise le frottement visqueux de l'air sur la balle par une force colinéaire (et opposée) au vecteur vitesse : $\vect{F_{\text{	Air}\rightarrow \text{Balle}}} = - \mu \vectv{G}{S}{\mathcal{R}} $. 

On a donc : 
$$ 
\vect{F_{\text{pesanteur}\rightarrow \text{balle}}} + \vect{F_{\text{Air}\rightarrow \text{Balle}}} 
= m \vectg{G}{S}{\mathcal{R}}
\Longleftrightarrow
 -mg\vect{y} - \mu \vectv{G}{S}{\mathcal{R}} =m \left[ \dfrac{d \vectv{G}{S}{\mathcal{R}}}{dt}\right]_{\mathcal{R}}
$$

En conséquence :
$$ 
 -mg\left[ \begin{array}{c}
0 \\ 1 \\ 0
...
En considérant que le mouvement plan, le mouvement de la balle est donc régit par le système d'équation différentielle suivant :

$$
\left\{
\begin{array}{l}
-\mu \dot{x}=m\ddot{x} \\
-mg -\mu \dot{y}=m\ddot{y}
\end{array}
\right.
$$
