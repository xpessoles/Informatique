...
$C_T(n) = C_e + n\left(2 C_e \right)$. Ici encore, la complexité de cet algorithme est linéaire car $C_T(n) \sim n$.
\end{exemple}
...
\begin{itemize}
\item au rang $1$, $C_1 = Ce + \left(n-1\right)\left( 2 C_e\right) + 3 C_e$;
\item au rang $2$, $C_2 = Ce + \left(n-2\right)\left( 2 C_e\right) + 3 C_e$;
\item au rang $i$, $C_i = Ce + \left(n-(i-1)\right)\left( 2 C_e\right) + 3 C_e$.
\end{itemize}

Le coût temporel peut donc s'exprimer ainsi : 
\begin{eqnarray*}
C_T(n) &=& \sum\limits_{i=0}^{n} \left(Ce + \left(n-(i-1)\right)\left( 2 C_e\right) + 3 C_e \right) = C_e\sum\limits_{i=0}^{n} \left( 4 +  2 n - 2 i + 2  \right) \\
&=& C_e \left( 6n + 2n^2 -2 \dfrac{n(n+1)}{2}\right) = 
C_e \left( 5n + n^2  \right)
%&=& C_e \left( 6 n \sum\limits_{i=0}^{n} \left( 4C_e +  2 n C_e - 2 i C_e + 2 C_e  \right) \\
\end{eqnarray*}
...
Dans ce cas, la difficulté consiste en déterminer le nombre de fois que sera exécutée la boucle \textsf{while}. On note $C_w = C_e + \max\left(C_e + C_r ; 2 C_e + 2 C_e; 3 C_e + C_e \right) = C_e + \max\left(C_e + C_r ; 4 C_e  \right)$.
