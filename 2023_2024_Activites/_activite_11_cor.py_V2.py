...
def cor_recherche_minimum(L : list, i : int) -> int :
...
def cor_inversion(L : list, i : int, j : int) -> None :
    L[i],L[j] = L[j],L[i]
    
def cor_tri_selection(L : list) -> None :
    for i in range(len(L)):
        m = cor_recherche_minimum(L,i)
        cor_inversion(L,i,m)
        
def cor_elts_inf(L : list, p : int) -> list :
    res = []
    for e in L :
        if e<p :
            res.append(e)
...
def cor_elts_sup(L : list, p : int) -> list :
    res = []
    for e in L :
        if e>=p :
            res.append(e)
...
def cor_tri_rapide(L : list) -> list :
    if len(L) == 0 :
        return []
    else : 
        p = L[-1]
        ei = cor_elts_inf(L,p)
        es = cor_elts_sup(L[:-1],p)
        return cor_tri_rapide(ei) + [p] + cor_tri_rapide(es)
